...
class GAN(object):
    def __init__(self, data, gen, num_steps, batch_size, log_every):
        ...
        self.mlp_hidden_size = 16  # 用神经网络模型当G D 网络时,隐层神经元个数
        self.count = 0
    ...
    def train(self):  # 1
        ...
        with tf.Session() as session:
            ...
            for step in range(self.num_steps):
                ...
                if step % 200 == 0 or step == 0 or step == self.num_steps - 1:
    ...
    def _plot_distributions(self, session):  # 2/100
        ...
        plt.legend()
        plt.savefig('{0}.png'.format(self.fig_id))
        self.fig_id += 1
        plt.close()
        # plt.show()
...
def parse_args():
    ...
    parser.add_argument('--num-steps', type=int, default=50000,  # 1200
