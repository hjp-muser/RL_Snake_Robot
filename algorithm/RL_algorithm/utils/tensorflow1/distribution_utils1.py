...
from algorithm.RL_algorithm.utils.tensorflow1.layer_utils import linear
...
class ProbabilityDistributionType(object):
    ...
    def proba_distribution_from_latent(self, pi_latent_vector, name_id=None, init_scale=1.0, init_bias=0.0):
        """
        returns the probability distribution from latent values

        :param pi_latent_vector: ([float]) the latent pi values
        :param name_id: (int) use to distinguish a different pi_latent_vector
...
class CategoricalProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_latent(self, pi_latent_vector, name_id=None, init_scale=1.0, init_bias=0.0):
        pdparam = linear(pi_latent_vector, 'pi', self.n_cat, name_id=name_id, init_scale=init_scale, init_bias=init_bias, use_bias=True)
...
class MultiCategoricalProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_latent(self, pi_latent_vector, name_id=None, init_scale=1.0, init_bias=0.0):
        pdparam = linear(pi_latent_vector, 'pi', sum(self.n_vec), name_id=name_id, init_scale=init_scale, init_bias=init_bias, use_bias=True)
        return self.proba_distribution_from_flat(pdparam)
...
class DiagGaussianProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_latent(self, pi_latent_vector, name_id=None, init_scale=1.0, init_bias=0.0):
        mean = linear(pi_latent_vector, 'pi', self.size, name_id=name_id, init_scale=init_scale, init_bias=init_bias, use_bias=True)
        # logstd = tf.get_variable(name='pi/logstd', shape=[1, self.size], initializer=tf.zeros_initializer())
        logstd = linear(pi_latent_vector, 'pi/logstd', self.size, name_id=name_id, init_scale=init_scale, use_bias=False)
...
class BernoulliProbabilityDistributionType(ProbabilityDistributionType):
    ...
    def proba_distribution_from_latent(self, pi_latent_vector, name_id=None, init_scale=1.0, init_bias=0.0):
        pdparam = linear(pi_latent_vector, 'pi', self.size, name_id=name_id, init_scale=init_scale, init_bias=init_bias, use_bias=True)
        return self.proba_distribution_from_flat(pdparam)
...
class BernoulliProbabilityDistribution(ProbabilityDistribution):
    ...
    def neglogp(self, x):
        return tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits,
                                                                     labels=tf.cast(x, tf.float32)), axis=-1)
