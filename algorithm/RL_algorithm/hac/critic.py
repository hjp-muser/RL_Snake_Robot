...
class Critic:
    ...
    def __init__(self, sess, env, num_layers, layer_number, attempt_limit, learning_rate=0.001, gamma=0.98, tau=0.05):
        self.sess = sess
        self.env = env
        ...
        if layer_number == 0:
            ...
            action_dim = self.env.task.subgoal_dim
    ...
    def update(self, old_states, old_actions, rewards, new_states, goals, new_actions, is_terminals):
        ...
        for i in range(len(wanted_qs)):
            ...
            assert 0 >= wanted_qs[i] >= self.q_limit, "Q-Value target not within proper bounds"
